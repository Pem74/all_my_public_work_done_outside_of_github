...
def generate_det_frames(url):
    ...
    # Ouvrir la vidéo
    cap = cv2.VideoCapture(f"http://{url}")
    if not cap.isOpened():
        print(f"Erreur lors de l'ouverture de la vidéo à l'URL: {url}")
        return
...
@app.route('/video_detect/<path:url_param>', methods=['GET'])
def video_detect(url_param):
    # Route for the video with detections
    return Response(generate_det_frames(url_param),
...
if __name__ == '__main__':
    # Démarrer le serveur Flask sur localhost et le port 4000
